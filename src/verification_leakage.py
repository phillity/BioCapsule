...
def verification(method, rs_cnt):
    ...
    for fold in range(10):
        ...
        train_fold_prev = lfw_prev["train_{}".format(fold)]
        X_train = np.vstack([train_fold[:, :-1], train_fold_prev[:, :-1]])
        y_train = np.vstack([train_fold[:, -1][..., np.newaxis],
                             train_fold_prev[:, -1][..., np.newaxis] + np.amax(train_fold[:, -1][..., np.newaxis])])
...
if __name__ == "__main__":
    ...
    np.savez_compressed(os.path.join(os.path.abspath(
        ""), "results", "{}_{}_lfw_leakage_true.npz".format(args["method"], args["rs_cnt"])), y_true)
    np.savez_compressed(os.path.join(os.path.abspath(
        ""), "results", "{}_{}_lfw_leakage_prob.npz".format(args["method"], args["rs_cnt"])), y_prob)
    np.savez_compressed(os.path.join(os.path.abspath(
        ""), "results", "{}_{}_lfw_leakage_acc.npz".format(args["method"], args["rs_cnt"])), acc)
